...
export default function SettingsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [configRes, hardwareRes] = await Promise.all([
      ...
      const [configData, hardwareData] = await Promise.all([
        configRes.json(),
        hardwareRes.json(),
      ]);
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#1a1a1f] rounded-xl border border-[#2a2a2f] overflow-hidden">
            <div className="px-6 py-4 border-b border-[#2a2a2f] flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold">Hardware Detection</h2>
                <p className="text-sm text-[#636366] mt-1">
                  Physical hardware currently detected on this system
                </p>
              </div>
              <button
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className={`p-4 rounded-lg border ${
                  hardware?.labjack_available
                    ...
                    : 'bg-[#0a0a0b] border-[#2a2a2f]'
                }`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${
                      hardware?.labjack_available ? 'bg-green-500 animate-pulse' : 'bg-[#636366]'
                    }`} />
                    <span className="font-medium">LabJack U3</span>
                  </div>
                  <p className="text-sm text-[#a1a1a6] mt-2">
                    {hardware?.labjack_available
                      ? hardware.labjack_details || 'Connected'
                      : 'Not detected'
                    }
                  </p>
                </div>
                ...
                <div className={`p-4 rounded-lg border ${
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${
                      hardware?.ola_available ? 'bg-green-500 animate-pulse' : 'bg-[#636366]'
                    }`} />
                    <span className="font-medium">OLA / DMX</span>
                  </div>
                  <p className="text-sm text-[#a1a1a6] mt-2">
                    {hardware?.ola_available
                      ? hardware.ola_details || 'Connected'
                      : 'Not detected'
                    }
                  </p>
                </div>
