...
const inputTypeLabels: Record<SwitchInputType, { label: string; description: string; color: string }> = {
  retractive: {
    label: 'Retractive',
    description: 'Momentary push button (spring return)',
    color: 'bg-blue-500/15 text-blue-400 border-blue-500/30'
  },
  rotary_abs: {
    label: 'Rotary Absolute',
    description: 'Rotary encoder with absolute position',
    color: 'bg-purple-500/15 text-purple-400 border-purple-500/30'
  },
  paddle_composite: {
    label: 'Paddle Composite',
    description: 'Up/down paddle with dimming',
...
const dimmingCurveLabels: Record<DimmingCurve, { label: string; description: string }> = {
  linear: { label: 'Linear', description: 'Direct proportional dimming' },
  logarithmic: { label: 'Logarithmic', description: 'Perceptually uniform dimming' },
};
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[#636366] mt-1">Define switch/dimmer types with input characteristics</p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-[#2a2a2f]">
              {models.map((model) => (
                <tr key={model.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm">{dimmingCurveLabels[model.dimming_curve].label}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      {model.requires_digital_pin && (
                        <span className="px-2 py-0.5 text-xs bg-blue-500/10 text-blue-400 rounded">Digital</span>
                      )}
                      {model.requires_analog_pin && (
                        <span className="px-2 py-0.5 text-xs bg-purple-500/10 text-purple-400 rounded">Analog</span>
                      )}
                    </div>
                  </td>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, input_type: e.target.value as SwitchInputType })}
                ...
                >
                  {Object.entries(inputTypeLabels).map(([value, { label, description }]) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="500"
                  />
                  <p className="text-xs text-[#636366] mt-1">Time to ignore repeated inputs</p>
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 bg-[#111113] border border-[#2a2a2f] rounded-lg text-white focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/50"
                  >
                    {Object.entries(dimmingCurveLabels).map(([value, { label }]) => (
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-[#a1a1a6]">Pin Requirements</label>
                <div className="space-y-2">
                  <label className="flex items-center gap-3 p-3 bg-[#111113] border border-[#2a2a2f] rounded-lg cursor-pointer hover:border-[#3a3a3f]">
                    <input
                      type="checkbox"
                    ...
                    />
                    <div>
                      <div className="font-medium text-sm">Requires Digital Pin</div>
                      <div className="text-xs text-[#636366]">LabJack FIO or EIO digital input</div>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 p-3 bg-[#111113] border border-[#2a2a2f] rounded-lg cursor-pointer hover:border-[#3a3a3f]">
                    <input
                    ...
                    />
                    <div>
                      <div className="font-medium text-sm">Requires Analog Pin</div>
                      <div className="text-xs text-[#636366]">LabJack AIN analog input (for encoders/potentiometers)</div>
                    </div>
                  </label>
                </div>
              </div>
